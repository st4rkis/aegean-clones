...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `${city} ${type} Airport (${code}) - Live Flight Departures`;

  // Description: Vibe A (Utility Focused)
  // Note: Changed "at" to "from" for grammatical correctness with departures
  const description = `View real-time flight status, delays, and terminal information for departures from ${city} ${type} Airport. Updated every minute.`;
